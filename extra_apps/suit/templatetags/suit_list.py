...
def suit_list_filter_select(cl, spec):
    ...
    for choice in choices:
       # 修改搜索框中的动作标志 Begin
        if choice['display'] == u'1':
            choice['display'] = u'新增'
        elif choice['display'] == u'2':
            choice['display'] = u'修改'
        elif choice['display'] == u'3':
            choice['display'] = u'删除'
       # 修改搜索框中的动作标志 End
...
def cells_handler(results, cl):
	...
	# 修改日志列表中动作标志的显示 Begin
        for result in results:
            if result[-1] == u'<td class="field-action_flag">1</td>':
                result[-1] = mark_safe(u'<td class=\"field-action_flag\">新增</td>')
            elif result[-1] == u'<td class="field-action_flag">2</td>':
                result[-1] = mark_safe(u'<td class=\"field-action_flag\">修改</td>')
            elif result[-1] == u'<td class="field-action_flag">3</td>':
                result[-1] = mark_safe(u'<td class=\"field-action_flag\">删除</td>')
	# 修改日志列表中动作标志的显示 End
        return results
